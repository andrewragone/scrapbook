...
class DQN():
    def __init__(self, model_lr=0.0001, epsilon=0.3, epsilon_min=0.05, epsilon_decay = 3000, total_episodes = 1000000,
                 memory_max_replay = 20000, mini_batch_size = 30, target_network_update_freq = 1000,
                 gym_game = 'CartPole-v0', gym_action_num = 2, gym_action_shift = 0,
                 render = False, loadWeights = False):
        '''
    ...
    def main(self):
        ...
        for episode in range(self.total_episodes):
            ...
            while done == False:
                ...
                reward = self.__process_reward(nextState=nextState, done=done)
                ...
                inputs, targets = expReplay.random_mini_batch(action_value = self.action_value, target_action_value=self.target_action_value)
            ...
            print(str(datetime.now()), expReplay.memory_size(), " Episode: ", episode, " epsilon ",  '%0.4f' % self.epsilon,
                  " loss ", '%0.5f' % loss, " step: ", stepCount, " total Steps ", totalSteps,
    ...
    def __create_nn_model(self):
        model = Sequential()
        model.add(Dense(32, input_dim=4, bias=True))
        model.add(Activation('relu'))
        model.add(Dense(64, bias=True))
        model.add(Activation('relu'))
        model.add(Dense(32, bias=True))
        model.add(Activation('relu'))
    ...
    def __process_reward(self, nextState, done):
        reward = math.cos(nextState[0][1])
        # cart_position = nextState[0][0]
        # if cart_position > 0.07 or cart_position < -0.07:
        #     reward -= 1.0
        return reward

    def __process_gym_state(self, gym_state):
        state = np.reshape(gym_state, (1,4))
        return state
...
class ExperienceReplay():
    ...
    def store_transition(self, state, action, reward, nextState, done):
        ...
        self.memory_state.append(state.reshape(4))
        self.memory_action.append(action)
        self.memory_reward.append(reward)
        self.memory_nextState.append(nextState.reshape(4))
    ...
    def random_mini_batch(self, action_value, target_action_value, gamma = 0.99):
        ...
        mini_batch_state = np.take(self.memory_state, randidx, axis=0)
        mini_batch_action = np.take(self.memory_action, randidx, axis=0)
        mini_batch_reward = np.take(self.memory_reward, randidx, axis=0)
        mini_batch_nextState = np.take(self.memory_nextState, randidx, axis=0)
        mini_batch_done = np.take(self.memory_done, randidx, axis=0)

        targets = action_value.predict(mini_batch_state)
        Qvalue = target_action_value.predict(mini_batch_nextState)
        for i, target in enumerate(targets):
            action = mini_batch_action[i]
            reward = mini_batch_reward[i]
        ...
        return mini_batch_state, targets
